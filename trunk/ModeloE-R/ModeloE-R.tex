...
\begin{tikzpicture}[node distance=2.5cm, every edge/.style={link}]
  ...
  \node[entity] (productor independiente) [right of= es provisto, node distance=14em] {PRODUCTOR\_INDEPENDIENTE} edge node[auto,swap] {[0..1]} (es provisto);
  ...
  \node[ident relationship] (se le asigna2) [right of=espacio, node distance=10em] {Se le asigna} edge [->] node[auto,swap] {[1..1]} (espacio);
  ...
  \node[attribute] (nro orden) [left of=se publicita, node distance=9em] {\key{Nro orden}} edge (se publicita);
