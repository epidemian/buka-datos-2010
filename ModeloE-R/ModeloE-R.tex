...
\begin{tikzpicture}[node distance=2.5cm, every edge/.style={link}]
  ...
  \node[relationship] (contiene) [above right=of programa] {Contiene} edge node[auto,swap] {[1..*]} (programa);
    
  \node[weak entity] (tanda) [right of=se le asigna2, node distance=10em] {Tanda} edge node[auto,swap] {[0..*]} (contiene);
  \draw (tanda) edge [total] node[auto,swap] {[0..*]} (se le asigna2);
  ...
  \node[relationship] (se publicita) [below right of=tanda, node distance=10em] {Se publicita} edge node[auto,swap] {[1..*]} (tanda);
  \node[attribute] (nro orden) [left of=se publicita, node distance=9em] {Nro orden} edge (se publicita);
  
  \node[entity] (anuncio) [below left of=se publicita, node distance=10em] {Anuncio} edge node[auto,swap] {[1..*]} (se publicita);
  \node[attribute] (producto ofrecido) [below right=of anuncio] {Producto ofrecido} edge (anuncio);
  \node[attribute] (titulo) [left=of anuncio] {\key{T\'itulo}} edge (anuncio);
  \node[attribute] (duracion) [right=of anuncio] {Duraci\'on} edge (anuncio);
   
  \node[relationship] (auspiciado) [below left of=anuncio, node distance=9em] {Auspiciado} edge node[auto,swap] {[1..*]} (anuncio);
  
  \node[entity] (anunciante) [below right of=auspiciado, node distance=10em] {Anunciante} edge node[auto,swap] {[0..1]} (auspiciado);
